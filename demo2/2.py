...
public class AdvancedSensorDataCleaner {
    ...
    // 数据库数据获取
    private static List<SensorData> fetchRawData() {
    ...
    // 核心清洗算法
    private static List<SensorData> cleanData(List<SensorData> rawData) {
        // 第一阶段：基础清洗
        ...
        List<SensorData> stage2 = new ArrayList<>();
    ...
    // 移动平均插值算法
    private static double calculateMovingAverage(List<SensorData> recentData, SensorData current) {
    ...
    // 数据保存
    private static void saveCleanedData(List<SensorData> cleanedData) {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
    ...
    // 生成清洗报告
    private static void generateCleaningReport(List<SensorData> cleanedData) {
        ...
        long outlierCount = cleanedData.stream()
                .filter(data -> data.outlierScore > 0)
                .count();
        System.out.printf("\n检测到异常值记录: %d 条 (占总数据的 %.2f%%)",
                outlierCount,
                (double) outlierCount / cleanedData.size() * 100);
    }
}